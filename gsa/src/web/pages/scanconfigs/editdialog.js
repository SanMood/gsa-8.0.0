...
const NvtPreferences = ({config, preferences, onEditNvtDetailsClick}) => {
  return (
    <Section foldable title={_('网络漏洞测试首选项')}>
      <Table fixed>
        <TableHeader>
          <TableRow>
            <TableHead width="30%">{_('NVT')}</TableHead>
            <TableHead width="30%">{_('名称')}</TableHead>
            <TableHead width="30%">{_('值')}</TableHead>
  ...
  render() {
    ...
    return (
      <TableRow key={name}>
        ...
        <TableData align="start">{_('{{count}} / {{max}}', counts)}</TableData>
...
class NvtFamilies extends React.Component {
  ...
  render() {
    ...
    return (
      <Section foldable title={_('编辑网络漏洞测试系列')}>
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>{_('系列')}</TableHead>
              <TableHead>{_('已选择的NVT')}</TableHead>
              <TableHead>{_('趋势')}</TableHead>
              <TableHead width="9em">{_('选择所有的NVT')}</TableHead>
              <TableHead align="center">{_('操作')}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {map(families, family => {
            ...
            <TableRow>
              <TableData>
                {_('总计：{{count}}', {count: config.families.count})}
              </TableData>
              <TableData align="start">
                {_('{{known}} / {{max}}', config.nvts)}
              </TableData>
