      ...
      <OvaldefClassLoader filter={filter}>
        {loaderProps => (
          <DataDisplay
            ...
            dataTitles={[_('分类'), _('# of OVAL Definitions')]}
            dataRow={row => [row.label, row.value]}
            title={({data: tdata}) =>
              _('按类别列出的OVAL定义（总计：{{count}})', {
                count: tdata.total,
              })
            }
            initialState={{
...
export const OvaldefClassTableDisplay = createDisplay({
  ...
  title: ({data: tdata}) =>
    _('按类别列出的OVAL定义（总计：{{count}})', {count: tdata.total}),
  dataTitles: [_l('分类'), _l('# of OVAL Defs')],
  dataRow: row => [row.label, row.value],
  dataTransform: transformClassData,
