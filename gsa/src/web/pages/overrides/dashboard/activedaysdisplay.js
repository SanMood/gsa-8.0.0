...
const transformActiveDaysData = (data = {}) => {
  ...
  const tdata = groups.map(group => {
    ...
    switch (value) {
      ...
      default:
        if (group.bulked) {
          label = _('有效期 > {{value}}天', {value});
        } else {
          label = _('剩余有效期：{{value}}天', {value});
        }
        break;
    }
    return {
...
export const OverridesActiveDaysTableDisplay = createDisplay({
  ...
  dataTitles: [_l('有效'), _l('# of Overrides')],
  dataTransform: transformActiveDaysData,
  title: ({data: tdata}) =>
    _('按活动时间列出复写（总计：{{count}}）', {count: tdata.total}),
