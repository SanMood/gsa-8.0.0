    ...
    if (isDefined(alert)) {
      ...
      Promise.all([credentialPromise, alertPromise]).then(
        ([credentials, settings]) => {
          ...
          if (event.type === '更新的安全信息已到达') {
            event_type = '新的安全信息已到达';
            feed_event = 'updated';
          } else {
            feed_event = 'new';
          }
          ...
          this.setState({
            ...
            tasks,
            title: _('编辑警报 {{name}}', {name: shorten(alert.name)}),
          });
        },
      );
    } else {
